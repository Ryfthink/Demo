...
public class WorksheetView extends View {
    ...
    public WorksheetView(Context context) {
		super(context);
		...
		switch (action & MotionEvent.ACTION_MASK) {
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if (pointerIndexTranslate >= 0) {
					...
					this.overScrollBy((int) (fTranslateLastTouch.x - translateX), (int) (fTranslateLastTouch.y - translateY), this.getScrollX(), this.getScrollY(), fWorksheetWidth - this.getWidth(),
							fWorksheetHeight - this.getHeight(), OVERSCROLL_DISTANCE, OVERSCROLL_DISTANCE, true);
	...
	public void computeScroll() {
		if (fScroller != null && fScroller.computeScrollOffset()) {
			...
			if (oldX != x || oldY != y) {
				this.overScrollBy(x - oldX, y - oldY, oldX, oldY, fWorksheetWidth - this.getWidth(), fWorksheetHeight - this.getHeight(), OVERSCROLL_DISTANCE, OVERSCROLL_DISTANCE, false);
			}
