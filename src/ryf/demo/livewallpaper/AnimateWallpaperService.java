...
public class AnimateWallpaperService extends WallpaperService {
	...
	class BokehEngine extends AnimationEngine {
		...
		void draw(Canvas c) {
			...
			synchronized (circles) {
				for (BokehRainbowCircle circle : circles) {
					...
					// paint the fill
					paint.setColor(Color.argb(circle.alpha, Color.red(circle.color), Color.green(circle.color), Color.blue(circle.color)));
					paint.setStyle(Paint.Style.FILL_AND_STROKE);
					c.drawCircle(circle.x + this.offsetX, circle.y + this.offsetY, circle.radius, paint);

					// paint the contour
					paint.setColor(Color.argb(circle.alpha, 63 + 3 * Color.red(circle.color) / 4, 63 + 3 * Color.green(circle.color) / 4, 63 + 3 * Color.blue(circle.color) / 4));
		...
		protected void iteration() {
			synchronized (circles) {
				for (Iterator<BokehRainbowCircle> it = circles.iterator(); it.hasNext();) {
		...
		int getColor(float yFraction) {
			return Color.HSVToColor(new float[] { 360.0f * yFraction, 1.0f, 1.0f });
		}
		...
		void createCircle(int x, int y) {
			...
			BokehRainbowCircle circle = new BokehRainbowCircle(x, y, radius, color, steps);
	...
	abstract class AnimationEngine extends Engine {
	...
	class BokehRainbowCircle {
		...
		public BokehRainbowCircle(float xCenter, float yCenter, float radius, int color, int steps) {
			this.x = xCenter;
			this.origX = xCenter;
